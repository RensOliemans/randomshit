...
def main():
    ...
    indices_of_first_messages_of_days = determine_indices_of_first_messages_per_day(
        all_messages
    )
    messages_per_day = split_messages_into_days(
        all_messages, indices_of_first_messages_of_days
    )

    print(
        f"Possible puzzles next to bonuspuzzles (a bonuspuzzle was found "
...
def determine_indices_of_first_messages_per_day(messages):
    ...
    for index, current_message in enumerate(messages):
        ...
        if message_is_first_of_the_day(
            current_message_datetime, previous_message_datetime
        ):
            yield index
...
def message_is_first_of_the_day(current_message_datetime, previous_message_datetime):
    ...
    previous_message_was_before_meeting = (
        previous_message_datetime.hour < MEETING_TIME.hour
    )
    current_message_datetime_was_after_meeting = (
        current_message_datetime.hour >= MEETING_TIME.hour
    )
    return (
        previous_message_was_before_meeting
        and current_message_datetime_was_after_meeting
    )
...
def convert_messages_to_puzzles(messages_per_day):
    ...
    for message in messages_per_day:
        ...
        puzzles_of_day.extend(
            convert_message_contents_to_puzzles(message_contents, message_date)
        )
...
def get_team_and_puzzle_number_from_message(message_text):
    ...
    if match is None:
        raise MessageIsOfIncorrectFormat(
            "Could not parse the message regex, was likely a kill "
...
def get_message_regex_match(message_text):
    try:
        ...
        raise MessageIsOfIncorrectFormat(
            "Message was of wrong type, was not a regular message"
