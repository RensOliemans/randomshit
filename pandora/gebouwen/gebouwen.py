...
parser = argparse.ArgumentParser(
    description="Get all buildings (and artworks) on the UT."
)
parser.add_argument(
    "-l",
    "--letters",
...
parser.add_argument(
    "-a", "--amount", type=int, help="how many letters does the building have"
)
parser.add_argument(
    "-b", "--begin", type=str, help="what are the first letters of the building?"
)
parser.add_argument(
    "-e", "--end", type=str, help="what are the last letters of the building?"
)
parser.add_argument("-m", "--middle", type=str, help="use: [<index><letter> ]+")
parser.add_argument(
    "-w",
    "--work-of-art",
    dest="include_artworks",
    action="store_true",
...
def apply_begin_filter(items, first_letters):
    return [
        x
        for x in items
        if x.name.lower()[: len(first_letters)] == first_letters.lower()
    ]
...
def apply_middle_filter(items, middle_letters):
    ...
    return [
        x
        for x in items
        if all(x.name.lower()[min(len(x) - 1, p)] == indices[p] for p in indices.keys())
    ]


def apply_end_filter(items, last_letters):
    return [
        x for x in items if x.name.lower()[-len(last_letters) :] == last_letters.lower()
    ]


def apply_some_letters_filter(items, some_letters):
    return [
        x for x in items if all([l in x.name.lower() for l in some_letters.lower()])
    ]
