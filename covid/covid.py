...
def main():
    ...
    for agegroup in agegroups:
        ...
        print(
            f"Age group {agegroup.ljust(5)} has {len(positive):n} positive cases and {len(deaths):n} deaths. "
            f"So {len(deaths) / len(positive):.3%} of the positively tested cases have died."
        )
    print(
        f"In total, {len(deceased) / len(df):.3%} of the positively tested cases have died."
    )
    print(
        "The stated percentage is incomplete and cannot be used as a chance to die from Covid. This is in part due "
